...
export function createAdmin(fetcher:FlickrFetch=request=>fetch(request)){
 ...
 app.get("/admin/login",async c=>{
  ...
  c.header("Cache-Control","no-store, no-transform");
  // Google's button frame needs the public site origin. This fixed, query-free
  // login route contains no session or transfer payload; other routes retain no-referrer.
  c.header("Referrer-Policy","strict-origin");
  // GIS copies the loader nonce to its generated button stylesheet. Style-only
  // authority leaves the external-script allowlist intact; no inline scripts are permitted.
  const styleNonce=Buffer.from(crypto.getRandomValues(new Uint8Array(24))).toString("base64");
  c.header("Content-Security-Policy",`default-src 'none'; script-src https://accounts.google.com/gsi/client; frame-src https://accounts.google.com; connect-src https://accounts.google.com; style-src 'self' https://accounts.google.com/gsi/style 'nonce-${styleNonce}'; form-action https://flickrgroupaddr.com/admin/google-login; base-uri 'none'; frame-ancestors 'none'`);
  return c.html(html`<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>Sign in to FGA</title><link rel="stylesheet" href="/admin/styles.css"><main class="login-page"><section class="login-card"><h1>Sign in to FGA</h1><p>Use your configured Google Account to manage your Flickr connection.</p><div id="g_id_onload" data-client_id="${c.env.GOOGLE_CLIENT_ID}" data-login_uri="https://flickrgroupaddr.com/admin/google-login" data-ux_mode="redirect" data-auto_prompt="false" data-auto_select="false" data-nonce="${tx.nonce}"></div><div class="g_id_signin" data-type="standard" data-state="${tx.state}"></div><script nonce="${styleNonce}" src="https://accounts.google.com/gsi/client" async defer></script></section></main></html>`);
 });
 ...
 app.get("/admin/signed-out",c=>{c.header("Cache-Control","no-store, no-transform");c.header("Content-Security-Policy","default-src 'none'; base-uri 'none'; frame-ancestors 'none'");return c.html(html`<!doctype html><html lang="en"><meta charset="utf-8"><title>Signed out of FGA</title><h1>Signed out of this FGA administrative session</h1><p>Your Google Account session, Flickr grant and installation credentials were not revoked.</p><a href="/admin/login">Sign in again</a></html>`);});
 ...
 app.get("/admin/",async c=>{
  ...
  const styleNonce=Buffer.from(crypto.getRandomValues(new Uint8Array(24))).toString("base64");
  const headers=new Headers(response.headers);headers.set("Cache-Control","no-store, no-transform");headers.set("Referrer-Policy","no-referrer");headers.set("Content-Security-Policy",`default-src 'self'; script-src 'self' https://accounts.google.com/gsi/client; style-src 'self' https://accounts.google.com/gsi/style 'nonce-${styleNonce}'; frame-src https://accounts.google.com; connect-src 'self' https://accounts.google.com; base-uri 'none'; object-src 'none'; frame-ancestors 'none'; form-action 'self'`);
  return new HTMLRewriter().on('script[src="app.mjs"]',{element(element){element.setAttribute("nonce",styleNonce);}}).transform(new Response(response.body,{status:response.status,headers}));
 });
