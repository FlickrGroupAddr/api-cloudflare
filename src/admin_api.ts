...
export function createAdmin(fetcher:FlickrFetch=request=>fetch(request)){
 ...
 app.get("/admin/login",async c=>{
  ...
  // authority leaves the existing external-script allowlist and referrer policy intact.
 ...
 app.get("/admin/",async c=>{
  ...
  const styleNonce=Buffer.from(crypto.getRandomValues(new Uint8Array(24))).toString("base64");
  const headers=new Headers(response.headers);headers.set("Cache-Control","no-store");headers.set("Referrer-Policy","no-referrer");headers.set("Content-Security-Policy",`default-src 'self'; script-src 'self' https://accounts.google.com/gsi/client; style-src 'self' https://accounts.google.com/gsi/style 'nonce-${styleNonce}'; frame-src https://accounts.google.com; connect-src 'self' https://accounts.google.com; base-uri 'none'; object-src 'none'; frame-ancestors 'none'; form-action 'self'`);
