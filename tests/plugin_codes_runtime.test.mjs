...
test("optimized production routes authenticate browser issuance and serialize native D1 credential rotation",async t=>{
 ...
 try{
  ...
  const login=await mf.dispatchFetch(origin+"/admin/google-login",{method:"POST",redirect:"manual",headers:{"CF-Connecting-IP":"192.0.2.10","Content-Type":"application/x-www-form-urlencoded",Cookie:"g_csrf_token=csrf"},body:new URLSearchParams({credential,state,g_csrf_token:"csrf"}).toString()});assert.equal(login.status,200);assert((await login.clone().text()).includes("/admin/login-complete.mjs"));
